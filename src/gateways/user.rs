...
impl<'a> TableGateway<'a> for UserTableGateway<'a> {
    ...
    fn find(self: &Self, id: Option<&str>) -> Result<Vec<Self::Model>, Self::Error> {
        if let Some(identity) = id {
            let mut sql_statement = self
                    ...
                    .unwrap();
            match sql_statement.query_row(&[identity], |row| {
                Ok(User {
    ...
    fn update(self: &Self, params: &Self::Params) -> Result<(), Self::Error> {
        match self
            .connection
            .prepare(include_str!("../sql/user/update.sql")) {
            Ok(mut statement) => match statement.execute(params.values()) {
                Ok(_) => Ok(()),
                Err(error) => Err(error),
            },
            Err(error) => Err(error),
